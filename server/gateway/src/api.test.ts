...
describe('Gateway API Tests', () => {
    ...
    it('should add a note', async () => {
        ...
        //Gets the new note to check its content
        response = await request(app).get('/api/note/5');
        expect(response.status).toBe(200);

        const expectedResults = JSON.parse(`
            {"id":"5","datetime":"2019-05-14T11:01:58.135Z","title":"Untitled","text":""}
            `);
    ...
    it('should save a note', async () => {
        ...
        const expectedResults = JSON.parse(`
            {"id":"1","datetime":"2020-03-01%10:10","title":"test title","text":"test text"}
            `);
        expect(response.text).toBe(JSON.stringify(expectedResults));
    });
