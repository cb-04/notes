...
describe('Gateway API Tests', () => {
  ...
  console.log('Response from /note/add:', response.status, response.text);
  expect(response.status).toBe(201);

  const noteId = response.text.trim(); // new ID returned (UUID string)

  // fetch the note by ID
  response = await request(app).get(`/api/note/${noteId}`);
  console.log('Response from /note/:id:', response.status, response.text);
  expect(response.status).toBe(200);

  // check the note's contents
  const note = JSON.parse(response.text);
